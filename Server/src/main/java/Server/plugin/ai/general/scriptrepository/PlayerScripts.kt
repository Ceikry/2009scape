...
object PlayerScripts {
    class PlayerScript(val identifier: String, val description: String, val name: String, val clazz: Class<*>)
    ...
    fun init(){
        ...
        result.getClassesWithAnnotation("plugin.ai.general.scriptrepository.PlayerCompatible").forEach { res ->
            val description = res.getAnnotationInfo("plugin.ai.general.scriptrepository.ScriptDescription").parameterValues[0].value.toString()
